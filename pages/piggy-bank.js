...
function PiggyBank() {
  ...
  useEffect(() => {
    const fetchAndUpdateLastSeenPiggyBank = async () => {
      try {
        ...
        const {data, error} = await supabase.from('user_connections').select('last_seen_piggy_at').filter('id', 'eq', user.id);
  ...
  useEffect(() => {
    const fetchStoriesAnsComments = async () => {
      try {
        const user = supabase.auth.user();
        const {data, error} = await supabase.from('stories').select('id, title, text, comments(id, text, created_at, comments_emotions(emotion_id))').filter('user_id', 'eq', user.id);
        if(error) throw error;
        if(data) {
          setStories(data);
        }
  ...
  return(
    <>
    {
      !loading && <MainLayout stories={stories} countedEmotions={countedEmotions} totalCommentsDone={totalCommentsDone} lastTimeSeen={lastTime}/>
    }
    </>
  ) 
}
